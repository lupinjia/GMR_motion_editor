...
- **步态周期检测**: 自动估计完整步态周期（同一脚两次触地间的时间，包括支撑相和摆动相）
...
编辑器自动检测并在主窗口显示步态周期时长（显示在机器人选择器旁）：

1. **什么是步态周期**:
   - 完整步态周期 = 从一次足跟着地到下一次同侧足跟着地的时间
   - 包括**支撑相**（脚接触地面）和**摆动相**（脚在空中摆动）
   - 这是步态分析中的标准定义
   - **窗口中显示的时间值为步态周期的一半**

2. **检测方法**:
   - 使用4种互补算法：
     - **位置极小值法**: 检测身体重心最低点（双腿支撑期）
     - **速度峰值法**: 检测最大向下速度（足跟着地时刻）
     - **加速度峰值法**: 检测触地冲击
     - **零交叉法**: 检测速度从负到正的零交叉
   - 通过加权平均融合各方法结果以提高鲁棒性
...
4. **显示格式**:
   - 小于1秒：以毫秒显示（如 "Gait: 820ms"）
   - 1秒或以上：以秒显示（如 "Gait: 1.25s"）
